...
class FFmpegConverter:
    ...
    def check_available(self) -> tuple[bool, str]:
        for binary in (self.ffmpeg_binary, self.ffprobe_binary):
            try:
                ...
                return False, f"{binary} was not found."
            if completed.returncode != 0:
                return False, f"{binary} is not responding correctly."
        return True, "FFmpeg is ready for conversion."
    ...
    def convert(
        ...
        try:
            ...
            return ConversionResult(False, f"Could not execute FFmpeg: {exc}", None)
        ...
        while True:
            if cancel_event is not None and cancel_event.is_set():
                self.cancel_current()
                return ConversionResult(False, "Conversion canceled by user.", None)
