...
class SettingsDialog(QDialog):
    ...
    def _t(self, key: str) -> str:
        return TEXTS.get(self._language_key, TEXTS["es"]).get(key, key)
...
class AudioTableWidget(QTableWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setHorizontalHeaderLabels(["", "Archivo", "Formato", "Duracion", "Tamano estimado", "Nombre de salida", "Estado", "Progreso"])
...
class MainWindow(QMainWindow):
    ...
    def _load_preferences(self) -> None:
        ...
        self.current_language = data.get("language", "es") if data.get("language") in TEXTS else "es"
    ...
    def _build_ui(self) -> None:
        ...
        self.browse_output_button.setIcon(self.style().standardIcon(QStyle.SP_DirOpenIcon))
        ...
        self.channels_combo.addItem("Estereo", 2)
        ...
        settings_layout.addWidget(self.bitrate_label, 1, 0)
        settings_layout.addWidget(self.bitrate_spin, 1, 1)
        settings_layout.addWidget(self.sample_rate_label, 1, 2)
        settings_layout.addWidget(self.sample_rate_combo, 1, 3)
        settings_layout.addWidget(self.channels_label, 1, 4)
        settings_layout.addWidget(self.channels_combo, 1, 5)
    ...
    def start_conversion(self) -> None:
        ...
        for row in range(self.table.rowCount()):
            ...
            if status_item:
                status_item.setText(self._t("state_queued"))
    ...
    def _on_row_started(self, row: int, output_path: str) -> None:
        status_item = self.table.item(row, 6)
        if status_item:
            status_item.setText(self._t("state_processing"))
        output_item = self.table.item(row, 5)
        if output_item:
    ...
    def _on_row_finished(self, row: int, success: bool, message: str) -> None:
        ...
        progress_bar = self.table.cellWidget(row, 7)
        if status_item:
            status_item.setText(self._t("state_done") if success else self._t("state_error"))
